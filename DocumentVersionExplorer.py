...
class DocumentVersionExplorer(QMainWindow):
    ...
    def setup_tray_icon(self):
        ...
        try:
            # First try to use the main window's icon
            main_icon = self.windowIcon()
            if not main_icon.isNull():
                self.tray_icon.setIcon(main_icon)
            else:
                # If no main icon, try to use a default icon from the theme
                icon = QIcon.fromTheme("document-open")
                if icon.isNull():
                    # If theme icon fails, use a built-in icon
                    icon = QIcon.fromTheme("application-x-executable")
                if icon.isNull():
                    # If all theme icons fail, create a simple icon
                    icon = QIcon()
                    # Create a 32x32 pixmap with a blue background
                    pixmap = QPixmap(32, 32)
                    pixmap.fill(Qt.GlobalColor.blue)
                    icon.addPixmap(pixmap)
                self.tray_icon.setIcon(icon)

            # Only try to load custom icon if it exists and is valid
    ...
    def setup_main_icon(self):
        try:
            # Try to use a default icon from the theme first
            icon = QIcon.fromTheme("document-open")
            if icon.isNull():
                # If theme icon fails, use a built-in icon
                icon = QIcon.fromTheme("application-x-executable")
    ...
    def changeEvent(self, event):
        ...
        if event.type() == 105:  # Qt.WindowStateChange
    ...
    def restore_from_tray(self):
        ...
        try:
            self.show()
            self.setWindowState(Qt.WindowState.Normal)
            self.activateWindow()
            self.raise_()
            self.log_action("INFO: Window restored from tray")
        except Exception as e:
            self.log_action(f"ERROR: Failed to restore window: {str(e)}")
            # Try a simpler restore if the full restore fails
            try:
                self.show()
                self.activateWindow()
            except Exception as e2:
                self.log_action(f"ERROR: Failed even basic window restore: {str(e2)}")
                QMessageBox.critical(self, "Error", "Failed to restore window. Please restart the application.")

    def quit_application(self):
        """Close only the DocumentVersionExplorer window."""
        self.log_action("INFO: Document Explorer closing.")
        self.tray_icon.hide()
        self.close()
